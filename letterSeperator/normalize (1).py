...
img = cv.imread('fox1.png', cv.IMREAD_COLOR_RGB)
...
def compare(c1, c2):
    ...
    if(between(y1,y2+h2/2,y1+h1) or between(y2,y1+h1/2,y2+h2)):
        if(x1<x2): 
            print("c1 smaller x")
            return -1
        else: 
...
s = sorted(horizontal_contours,key=cmp_to_key(compare))
...
for cont in s:
    ...
    resized = cv.resize(crop,(32,32),None,interpolation=cv.INTER_LINEAR)
    #plt.subplot(1,2,1),plt.imshow(copy)
    #plt.subplot(1,2,2),plt.imshow(crop)
    cv.imwrite("./output/cont"+str(iter)+".png",resized, [cv.IMWRITE_PNG_BILEVEL, 1])
