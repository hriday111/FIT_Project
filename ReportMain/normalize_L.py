...
img = cv.imread('ReportMain/input/L1.png', cv.IMREAD_COLOR_RGB)
...
for cont in horizontal_contours:
    ...
    #crop = cv.erode(crop,erosion_kernel2,iterations=1)
    if cv.imwrite("ReportMain/output/cont"+str(iter)+".png",crop, [cv.IMWRITE_PNG_BILEVEL, 1]) == True:
...
cv.imwrite("ReportMain/output/L1.png",img)
